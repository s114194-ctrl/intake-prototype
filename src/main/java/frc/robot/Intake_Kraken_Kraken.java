...
public class Intake_Kraken_Kraken extends SubsystemBase {
  ...
  private final MotionMagicVoltage run = new MotionMagicVoltage(0).withSlot(1);
  ...
  public void deployOut() {
    deploy.setControl(run.withPosition(Intake_Constants.Intake_Kraken_Deploy.Deploy_Start));
  }

  public void deployBack() {
    deploy.setControl(run.withPosition(Intake_Constants.Intake_Kraken_Deploy.Deploy_Back));
    //deploy.setControl(new MotionMagicVoltage(Intake_Constants.Intake_Kraken_Deploy.Deploy_Back).withPosition(-5).withSlot(1));
  }
  public void stop(){
    deploy.stopMotor();
  }
